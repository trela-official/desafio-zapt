export const dataCommentsSocial = [
  {
    userName: '@ferri-gui',
    img: 'avatar-man.jpg',
    comment:
      '”Não da para parar de comer.. esse final de semana pedi 20 pasteis da Ilma e acabaram TODOS em um só dia... O pessoal aqui de casa ficou viciado de um nível que terei que comprar toda semana... Assim não tem academia que secura rsrsrsrs Obrigado Zapt.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@LiciaR.',
    img: 'avatar-woman.jpg',
    comment:
      '”Nunca achara os produtos na Lush depois que ela saiu do Brasil.. só a Zapt para conseguir importá-los com preço mais barato do que vendiam aqui! Muito obrigado, recomendo sempre a todos os amigos.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@gust',
    img: 'avatar-man.jpg',
    comment:
      '”Achei uma promoção incrível de roupinha de bebê, foi o que salvou o enxoval do Pedro.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@paolaC',
    img: 'avatar-woman.jpg',
    comment:
      '”Estava precisando achar algum lugar que vendia produtos que veem direto da fazenda... a Zapt salvou.. e ainda paguei um preço muito mais barato que supermercado 🙌”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@shitz',
    img: 'avatar-man.jpg',
    comment:
      '”Ontem chegou uma pizza que comprei na Zapt e me lembrou as que minha avó fazia... sensacional!”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@Cookie',
    img: 'avatar-woman.jpg',
    comment:
      '”Meu dono finalmente conseguiu achar uma comida que eu gosto! Agora vou conseguir engordar e ser feliz.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@Galvão',
    img: 'avatar-man.jpg',
    comment:
      '”Todos os vinhos da vinícula Bueno estão disponíveis no catálogo da Zapt a um preço incrível... não percam essa oportunidade...”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@rafael',
    img: 'avatar-man.jpg',
    comment:
      '”Nossa cada dia tem uma oferta mais legal do que a outra.. Além de produtos de qualidade o preço é imbatível”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@eduque',
    img: 'avatar-man.jpg',
    comment:
      '“Não conhecia a Zapt.. um amigo meu indicou... falou para eu dar uma olhada nos preços das cervejas artesanais.. incrível.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@ferri-gui',
    img: 'avatar-man.jpg',
    comment:
      '”Não da para parar de comer.. esse final de semana pedi 20 pasteis da Ilma e acabaram TODOS em um só dia... O pessoal aqui de casa ficou viciado de um nível que terei que comprar toda semana... Assim não tem academia que secura rsrsrsrs Obrigado Zapt.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@LiciaR.',
    img: 'avatar-woman.jpg',
    comment:
      '”Nunca achara os produtos na Lush depois que ela saiu do Brasil.. só a Zapt para conseguir importá-los com preço mais barato do que vendiam aqui! Muito obrigado, recomendo sempre a todos os amigos.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@gust',
    img: 'avatar-man.jpg',
    comment:
      '”Achei uma promoção incrível de roupinha de bebê, foi o que salvou o enxoval do Pedro.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@paolaC',
    img: 'avatar-woman.jpg',
    comment:
      '”Estava precisando achar algum lugar que vendia produtos que veem direto da fazenda... a Zapt salvou.. e ainda paguei um preço muito mais barato que supermercado 🙌”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@shitz',
    img: 'avatar-man.jpg',
    comment:
      '”Ontem chegou uma pizza que comprei na Zapt e me lembrou as que minha avó fazia... sensacional!”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@Cookie',
    img: 'avatar-woman.jpg',
    comment:
      '”Meu dono finalmente conseguiu achar uma comida que eu gosto! Agora vou conseguir engordar e ser feliz.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@Galvão',
    img: 'avatar-man.jpg',
    comment:
      '”Todos os vinhos da vinícula Bueno estão disponíveis no catálogo da Zapt a um preço incrível... não percam essa oportunidade...”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@rafael',
    img: 'avatar-man.jpg',
    comment:
      '”Nossa cada dia tem uma oferta mais legal do que a outra.. Além de produtos de qualidade o preço é imbatível”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@eduque',
    img: 'avatar-man.jpg',
    comment:
      '“Não conhecia a Zapt.. um amigo meu indicou... falou para eu dar uma olhada nos preços das cervejas artesanais.. incrível.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@ferri-gui',
    img: 'avatar-man.jpg',
    comment:
      '”Não da para parar de comer.. esse final de semana pedi 20 pasteis da Ilma e acabaram TODOS em um só dia... O pessoal aqui de casa ficou viciado de um nível que terei que comprar toda semana... Assim não tem academia que secura rsrsrsrs Obrigado Zapt.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@LiciaR.',
    img: 'avatar-woman.jpg',
    comment:
      '”Nunca achara os produtos na Lush depois que ela saiu do Brasil.. só a Zapt para conseguir importá-los com preço mais barato do que vendiam aqui! Muito obrigado, recomendo sempre a todos os amigos.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@gust',
    img: 'avatar-man.jpg',
    comment:
      '”Achei uma promoção incrível de roupinha de bebê, foi o que salvou o enxoval do Pedro.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@paolaC',
    img: 'avatar-woman.jpg',
    comment:
      '”Estava precisando achar algum lugar que vendia produtos que veem direto da fazenda... a Zapt salvou.. e ainda paguei um preço muito mais barato que supermercado 🙌”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@shitz',
    img: 'avatar-man.jpg',
    comment:
      '”Ontem chegou uma pizza que comprei na Zapt e me lembrou as que minha avó fazia... sensacional!”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@Cookie',
    img: 'avatar-woman.jpg',
    comment:
      '”Meu dono finalmente conseguiu achar uma comida que eu gosto! Agora vou conseguir engordar e ser feliz.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@Galvão',
    img: 'avatar-man.jpg',
    comment:
      '”Todos os vinhos da vinícula Bueno estão disponíveis no catálogo da Zapt a um preço incrível... não percam essa oportunidade...”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@rafael',
    img: 'avatar-man.jpg',
    comment:
      '”Nossa cada dia tem uma oferta mais legal do que a outra.. Além de produtos de qualidade o preço é imbatível”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@eduque',
    img: 'avatar-man.jpg',
    comment:
      '“Não conhecia a Zapt.. um amigo meu indicou... falou para eu dar uma olhada nos preços das cervejas artesanais.. incrível.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@ferri-gui',
    img: 'avatar-man.jpg',
    comment:
      '”Não da para parar de comer.. esse final de semana pedi 20 pasteis da Ilma e acabaram TODOS em um só dia... O pessoal aqui de casa ficou viciado de um nível que terei que comprar toda semana... Assim não tem academia que secura rsrsrsrs Obrigado Zapt.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@LiciaR.',
    img: 'avatar-woman.jpg',
    comment:
      '”Nunca achara os produtos na Lush depois que ela saiu do Brasil.. só a Zapt para conseguir importá-los com preço mais barato do que vendiam aqui! Muito obrigado, recomendo sempre a todos os amigos.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@gust',
    img: 'avatar-man.jpg',
    comment:
      '”Achei uma promoção incrível de roupinha de bebê, foi o que salvou o enxoval do Pedro.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@paolaC',
    img: 'avatar-woman.jpg',
    comment:
      '”Estava precisando achar algum lugar que vendia produtos que veem direto da fazenda... a Zapt salvou.. e ainda paguei um preço muito mais barato que supermercado 🙌”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@shitz',
    img: 'avatar-man.jpg',
    comment:
      '”Ontem chegou uma pizza que comprei na Zapt e me lembrou as que minha avó fazia... sensacional!”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@Cookie',
    img: 'avatar-woman.jpg',
    comment:
      '”Meu dono finalmente conseguiu achar uma comida que eu gosto! Agora vou conseguir engordar e ser feliz.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@Galvão',
    img: 'avatar-man.jpg',
    comment:
      '”Todos os vinhos da vinícula Bueno estão disponíveis no catálogo da Zapt a um preço incrível... não percam essa oportunidade...”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@rafael',
    img: 'avatar-man.jpg',
    comment:
      '”Nossa cada dia tem uma oferta mais legal do que a outra.. Além de produtos de qualidade o preço é imbatível”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@eduque',
    img: 'avatar-man.jpg',
    comment:
      '“Não conhecia a Zapt.. um amigo meu indicou... falou para eu dar uma olhada nos preços das cervejas artesanais.. incrível.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@ferri-gui',
    img: 'avatar-man.jpg',
    comment:
      '”Não da para parar de comer.. esse final de semana pedi 20 pasteis da Ilma e acabaram TODOS em um só dia... O pessoal aqui de casa ficou viciado de um nível que terei que comprar toda semana... Assim não tem academia que secura rsrsrsrs Obrigado Zapt.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@LiciaR.',
    img: 'avatar-woman.jpg',
    comment:
      '”Nunca achara os produtos na Lush depois que ela saiu do Brasil.. só a Zapt para conseguir importá-los com preço mais barato do que vendiam aqui! Muito obrigado, recomendo sempre a todos os amigos.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@gust',
    img: 'avatar-man.jpg',
    comment:
      '”Achei uma promoção incrível de roupinha de bebê, foi o que salvou o enxoval do Pedro.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@paolaC',
    img: 'avatar-woman.jpg',
    comment:
      '”Estava precisando achar algum lugar que vendia produtos que veem direto da fazenda... a Zapt salvou.. e ainda paguei um preço muito mais barato que supermercado 🙌”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@shitz',
    img: 'avatar-man.jpg',
    comment:
      '”Ontem chegou uma pizza que comprei na Zapt e me lembrou as que minha avó fazia... sensacional!”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@Cookie',
    img: 'avatar-woman.jpg',
    comment:
      '”Meu dono finalmente conseguiu achar uma comida que eu gosto! Agora vou conseguir engordar e ser feliz.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@Galvão',
    img: 'avatar-man.jpg',
    comment:
      '”Todos os vinhos da vinícula Bueno estão disponíveis no catálogo da Zapt a um preço incrível... não percam essa oportunidade...”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@rafael',
    img: 'avatar-man.jpg',
    comment:
      '”Nossa cada dia tem uma oferta mais legal do que a outra.. Além de produtos de qualidade o preço é imbatível”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@eduque',
    img: 'avatar-man.jpg',
    comment:
      '“Não conhecia a Zapt.. um amigo meu indicou... falou para eu dar uma olhada nos preços das cervejas artesanais.. incrível.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@ferri-gui',
    img: 'avatar-man.jpg',
    comment:
      '”Não da para parar de comer.. esse final de semana pedi 20 pasteis da Ilma e acabaram TODOS em um só dia... O pessoal aqui de casa ficou viciado de um nível que terei que comprar toda semana... Assim não tem academia que secura rsrsrsrs Obrigado Zapt.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@LiciaR.',
    img: 'avatar-woman.jpg',
    comment:
      '”Nunca achara os produtos na Lush depois que ela saiu do Brasil.. só a Zapt para conseguir importá-los com preço mais barato do que vendiam aqui! Muito obrigado, recomendo sempre a todos os amigos.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@gust',
    img: 'avatar-man.jpg',
    comment:
      '”Achei uma promoção incrível de roupinha de bebê, foi o que salvou o enxoval do Pedro.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@paolaC',
    img: 'avatar-woman.jpg',
    comment:
      '”Estava precisando achar algum lugar que vendia produtos que veem direto da fazenda... a Zapt salvou.. e ainda paguei um preço muito mais barato que supermercado 🙌”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@shitz',
    img: 'avatar-man.jpg',
    comment:
      '”Ontem chegou uma pizza que comprei na Zapt e me lembrou as que minha avó fazia... sensacional!”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@Cookie',
    img: 'avatar-woman.jpg',
    comment:
      '”Meu dono finalmente conseguiu achar uma comida que eu gosto! Agora vou conseguir engordar e ser feliz.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@Galvão',
    img: 'avatar-man.jpg',
    comment:
      '”Todos os vinhos da vinícula Bueno estão disponíveis no catálogo da Zapt a um preço incrível... não percam essa oportunidade...”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@rafael',
    img: 'avatar-man.jpg',
    comment:
      '”Nossa cada dia tem uma oferta mais legal do que a outra.. Além de produtos de qualidade o preço é imbatível”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@eduque',
    img: 'avatar-man.jpg',
    comment:
      '“Não conhecia a Zapt.. um amigo meu indicou... falou para eu dar uma olhada nos preços das cervejas artesanais.. incrível.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@ferri-gui',
    img: 'avatar-man.jpg',
    comment:
      '”Não da para parar de comer.. esse final de semana pedi 20 pasteis da Ilma e acabaram TODOS em um só dia... O pessoal aqui de casa ficou viciado de um nível que terei que comprar toda semana... Assim não tem academia que secura rsrsrsrs Obrigado Zapt.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@LiciaR.',
    img: 'avatar-woman.jpg',
    comment:
      '”Nunca achara os produtos na Lush depois que ela saiu do Brasil.. só a Zapt para conseguir importá-los com preço mais barato do que vendiam aqui! Muito obrigado, recomendo sempre a todos os amigos.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@gust',
    img: 'avatar-man.jpg',
    comment:
      '”Achei uma promoção incrível de roupinha de bebê, foi o que salvou o enxoval do Pedro.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@paolaC',
    img: 'avatar-woman.jpg',
    comment:
      '”Estava precisando achar algum lugar que vendia produtos que veem direto da fazenda... a Zapt salvou.. e ainda paguei um preço muito mais barato que supermercado 🙌”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@shitz',
    img: 'avatar-man.jpg',
    comment:
      '”Ontem chegou uma pizza que comprei na Zapt e me lembrou as que minha avó fazia... sensacional!”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@Cookie',
    img: 'avatar-woman.jpg',
    comment:
      '”Meu dono finalmente conseguiu achar uma comida que eu gosto! Agora vou conseguir engordar e ser feliz.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@Galvão',
    img: 'avatar-man.jpg',
    comment:
      '”Todos os vinhos da vinícula Bueno estão disponíveis no catálogo da Zapt a um preço incrível... não percam essa oportunidade...”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@rafael',
    img: 'avatar-man.jpg',
    comment:
      '”Nossa cada dia tem uma oferta mais legal do que a outra.. Além de produtos de qualidade o preço é imbatível”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@eduque',
    img: 'avatar-man.jpg',
    comment:
      '“Não conhecia a Zapt.. um amigo meu indicou... falou para eu dar uma olhada nos preços das cervejas artesanais.. incrível.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@ferri-gui',
    img: 'avatar-man.jpg',
    comment:
      '”Não da para parar de comer.. esse final de semana pedi 20 pasteis da Ilma e acabaram TODOS em um só dia... O pessoal aqui de casa ficou viciado de um nível que terei que comprar toda semana... Assim não tem academia que secura rsrsrsrs Obrigado Zapt.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@LiciaR.',
    img: 'avatar-woman.jpg',
    comment:
      '”Nunca achara os produtos na Lush depois que ela saiu do Brasil.. só a Zapt para conseguir importá-los com preço mais barato do que vendiam aqui! Muito obrigado, recomendo sempre a todos os amigos.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@gust',
    img: 'avatar-man.jpg',
    comment:
      '”Achei uma promoção incrível de roupinha de bebê, foi o que salvou o enxoval do Pedro.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@paolaC',
    img: 'avatar-woman.jpg',
    comment:
      '”Estava precisando achar algum lugar que vendia produtos que veem direto da fazenda... a Zapt salvou.. e ainda paguei um preço muito mais barato que supermercado 🙌”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@shitz',
    img: 'avatar-man.jpg',
    comment:
      '”Ontem chegou uma pizza que comprei na Zapt e me lembrou as que minha avó fazia... sensacional!”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@Cookie',
    img: 'avatar-woman.jpg',
    comment:
      '”Meu dono finalmente conseguiu achar uma comida que eu gosto! Agora vou conseguir engordar e ser feliz.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@Galvão',
    img: 'avatar-man.jpg',
    comment:
      '”Todos os vinhos da vinícula Bueno estão disponíveis no catálogo da Zapt a um preço incrível... não percam essa oportunidade...”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@rafael',
    img: 'avatar-man.jpg',
    comment:
      '”Nossa cada dia tem uma oferta mais legal do que a outra.. Além de produtos de qualidade o preço é imbatível”',
    urlProfile: '/comentarios_nas_redes_sociais'
  },
  {
    userName: '@eduque',
    img: 'avatar-man.jpg',
    comment:
      '“Não conhecia a Zapt.. um amigo meu indicou... falou para eu dar uma olhada nos preços das cervejas artesanais.. incrível.”',
    urlProfile: '/comentarios_nas_redes_sociais'
  }
]
